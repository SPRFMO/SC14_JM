...
//---------------------------------------------------------------------------
 ...
 //  init_3darray log_selcoffs_fsh(1,nfsh,1,n_sel_ch_fsh,1,nselages_fsh,phase_selcoff_fsh)
  ...
  matrix sel_fut(1,nfsh,1,nages)
  matrix sel_msy(1,nfsh,1,nages)
  matrix avgsel_fsh(1,nfsh,1,n_sel_ch_fsh);

  matrix Ftot_fut(1,nstk,1,nages)
  matrix seltmp(1,nfsh,1,nages);
...
//---------------------------------NEW
  ...
  vector repl_target_SSB(1,nstk)
  vector repl_SSB_residual(1,nstk)
  vector repl_status(1,nstk)
  ...
  // Preserve scalar references independently of the annual reporting scratch values.
  sdreport_vector Fmsy_global(1,nstk)
  sdreport_vector Bmsy_global(1,nstk)
  sdreport_vector MSY_global(1,nstk)
  vector Rmsy_global(1,nstk)
  vector lnFmsy_global(1,nstk)
  vector Fratio_global(1,nfsh)
  // Projection reference uses sel_fut and terminal natural mortality.
  sdreport_vector Fmsy_proj(1,nstk)
  sdreport_vector Bmsy_proj(1,nstk)
  sdreport_vector MSY_proj(1,nstk)
  vector msy_proj_status(1,nstk)
...
FUNCTION write_mceval
  ...
  {
    // REFPTS
    mceval<< mc_count<<" MSY    "<<k<<" all "<<" all "<<MSY_global(k)  <<  endl;
    mceval<< mc_count<<" SBMSY  "<<k<<" all "<<" all "<<Bmsy_global(k) <<  endl;
    mceval<< mc_count<<" FMSY   "<<k<<" all "<<" all "<<Fmsy_global(k) <<  endl;
...
FUNCTION dvar_matrix ALK(dvar_vector& mu, dvar_vector& sig, dvector& x)
  ...
  return(pdf);
//---------------------------------------------------------------------------
FUNCTION dvariable repl_ssb(dvariable& Ftrial,const int& stock)
  // Catch is taken in T+1. Return SSB(T+2), and record SSB(T+1) as
  // the target under the same trial fishing pattern (Annex K timing).
  dvar_vector nstart(1,nages), nnext(1,nages), z(1,nages);
  dvar_matrix ff(1,nfsh,1,nages);
  ff.initialize(); nstart.initialize(); nnext.initialize();
  nstart(2,nages)=++elem_prod(natage(stock,endyr)(1,nages-1),S(stock,endyr)(1,nages-1));
  nstart(nages)+=natage(stock,endyr,nages)*S(stock,endyr,nages);
  int regime=cum_regs(stock)+yy_sr(stock,endyr);
  nstart(1)=SRecruit(Sp_Biom(stock,endyr),regime);
  double fscale=0.;
  for (int fleet=1;fleet<=nfsh;fleet++)
    if (sel_map(1,fleet)==stock) fscale+=value(mean(F(fleet,endyr)));
  z=M(stock,endyr);
  if (fscale>0.)
    for (int fleet=1;fleet<=nfsh;fleet++)
      if (sel_map(1,fleet)==stock)
      {
        ff(fleet)=Ftrial*sel_fut(fleet)*mean(F(fleet,endyr))/fscale;
        z+=ff(fleet);
      }
  dvar_vector surv=mfexp(-z);
  repl_target_SSB(stock)=elem_prod(nstart,pow(surv,spmo_frac))*wt_mature(stock);
  repl_yld(stock)=0.;
  for (int fleet=1;fleet<=nfsh;fleet++)
    if (sel_map(1,fleet)==stock)
      repl_yld(stock)+=sum(elem_prod(wt_fsh(fleet,endyr),
        elem_prod(nstart,elem_prod(elem_div(ff(fleet),z),1.-surv))));
  nnext(2,nages)=++elem_prod(nstart(1,nages-1),surv(1,nages-1));
  nnext(nages)+=nstart(nages)*surv(nages);
  nnext(1)=SRecruit(repl_target_SSB(stock),regime);
  return elem_prod(nnext,pow(surv,spmo_frac))*wt_mature(stock);

FUNCTION Get_Replacement_Yield
  // Solve SSB(T+2)-SSB(T+1)=0 using projection SRR and sel_fut.
  // Status 1 is an attained root. Negative codes are diagnostic boundaries:
  // -1 declining at zero F, -2 no root below F=10, -3 no fishing pattern,
  // -4 no convergence, -5 nonpositive target. Do not use boundary catch as advice.
  const double max_f=10.;
  repl_F.initialize(); repl_yld.initialize(); repl_SSB.initialize();
  repl_target_SSB.initialize(); repl_SSB_residual.initialize(); repl_status.initialize();
  for (int stock=1;stock<=nstk;stock++)
  {
    double scale=0.;
    for (int fleet=1;fleet<=nfsh;fleet++)
      if (sel_map(1,fleet)==stock) scale+=value(mean(F(fleet,endyr)));
    dvariable trial=0.;
    dvariable next_ssb=repl_ssb(trial,stock);
    double residual=value(next_ssb-repl_target_SSB(stock));
    double target=value(repl_target_SSB(stock));
    const double tol=1.e-8*(target>1. ? target : 1.);
    if (target<=0.) repl_status(stock)=-5;
    else if (scale<=0.) repl_status(stock)=-3;
    else if (fabs(residual)<=tol) repl_status(stock)=1;
    else if (residual<0.) repl_status(stock)=-1;
    else
    {
      double lo=0., hi=0.05;
      trial=hi;
      next_ssb=repl_ssb(trial,stock);
      residual=value(next_ssb-repl_target_SSB(stock));
      while (residual>tol && hi<max_f)
      {
        lo=hi; hi*=2.; if (hi>max_f) hi=max_f;
        trial=hi;
        next_ssb=repl_ssb(trial,stock);
        residual=value(next_ssb-repl_target_SSB(stock));
      }
      if (residual>tol) repl_status(stock)=-2;
      else
      {
        repl_status(stock)=-4;
        for (int iteration=0;iteration<100;iteration++)
        {
          ...
          next_ssb=repl_ssb(trial,stock);
          residual=value(next_ssb-repl_target_SSB(stock));
          if (fabs(residual)<=tol) { repl_status(stock)=1; break; }
          if (residual>0.) lo=value(trial); else hi=value(trial);
        }
      }
    }
    repl_F(stock)=trial;
    repl_SSB(stock)=repl_ssb(trial,stock);
    repl_SSB_residual(stock)=repl_SSB(stock)-repl_target_SSB(stock);
  }
...
FUNCTION Calc_Dependent_Vars
  ...
  for (k=1;k<=nfsh;k++)
  { 
    Fatmp(k)            = (Fratio_global(k) * Fmsy_proj(sel_map(1,k)) * sel_fut(k));
...
FUNCTION void get_future_sel()
    if (msy_sel<1 || msy_sel>endyr-styr || fut_sel<0 || fut_sel>endyr-styr)
    {
      cerr << "Invalid selectivity averaging window: msy_sel must be 1..endyr-styr; fut_sel 0..endyr-styr" << endl;
      ad_exit(1);
    }
    f_tmp.initialize();
    Ftot_fut.initialize();
    ...
    sel_msy.initialize();
		for (k=1;k<=nfsh;k++) 
		{
			...
			for (int ii=endyr-msy_sel; ii<endyr; ii++)
			{
			  sel_msy(k)    += (sel_fsh(k,ii));
			}
			sel_msy(k)    /= msy_sel;
		}

    // Get future selectivity based on -fut_sel flag
		if (fut_sel)
		{
			// pick up a scale factor (annual "effort") from terminal year
      for (k=1;k<=nfsh;k++) 
			{
				f_tmp(k) = mean(F(k,endyr));
				for (int ii=endyr-fut_sel; ii<endyr; ii++)
				{
				  sel_fut(k)    += (sel_fsh(k,ii));
				}
				sel_fut(k)    /= fut_sel;
      ...
      for (k=1;k<=nfsh;k++)
		...
		//cout<<"Future selex"<<endl;
		//cout << sel_fut <<endl; ad_exit(1);
...
FUNCTION void get_future_Fs(const int& s,const int& i,const int& iscenario)
    ...
    dvector p_lastyr(1,nfsh);
      ...
      for (k=1;k<=nfsh;k++)
		...
		// cout << Ftot_fut(s) <<endl<<F_fut_tmp<<endl; ad_exit(1);
    ...
    switch (iscenario)
    {
      ...
      case 3:
        ...
        break;
      case 4:
        // Optimize and apply Fmsy with the same selectivity as other projections.
        for (int k=1;k<=nfsh;k++) F_fut_tmp(k) = sel_fut(k)*Fratio_global(k)*Fmsy_proj(s);
        break;
      case 5:
 ...
 /*Function calculates used in calculating MSY and MSYL for a designated component of the
  ...
  for (s=1;s<=nstk;s++)
  {
    ...
    }
    {
      ...
      Rmsy(s)     = Rtmp(s);
      Fmsy_global(s) = Fmsy(s);
      Bmsy_global(s) = Bmsy(s);
      MSY_global(s) = MSY(s);
      Rmsy_global(s) = Rmsy(s);
      lnFmsy_global(s) = lnFmsy(s);
    }
  }
  Fratio_global = Fratio;

FUNCTION void restore_global_msy()
  Fmsy=Fmsy_global;
  Bmsy=Bmsy_global;
  MSY=MSY_global;
  Rmsy=Rmsy_global;
  lnFmsy=lnFmsy_global;
  Fratio=Fratio_global;

FUNCTION dvar_vector projection_equilibrium(const dvariable& f,const int& stock)
  RETURN_ARRAYS_INCREMENT();
  // Same fleet shares, selectivity, mortality and weights as scenario 4.
  dvar_vector ans(1,3);
  dvar_vector n(1,nages);
  dvar_vector z(1,nages);
  dvar_matrix ff(1,nfsh,1,nages);
  ff.initialize(); ans.initialize();
  z=M(stock,endyr);
  for (int fleet=1;fleet<=nfsh;fleet++)
    if (sel_map(1,fleet)==stock)
    {
      ff(fleet)=f*Fratio_global(fleet)*sel_fut(fleet);
      z+=ff(fleet);
    }
  dvar_vector surv=mfexp(-z);
  n(1)=1.;
  for (int age=1;age<nages;age++) n(age+1)=n(age)*surv(age);
  n(nages)/=1.-surv(nages);
  dvariable phi=elem_prod(n,pow(surv,spmo_frac))*wt_mature(stock);
  dvariable req=Requil(phi,endyr,stock);
  ans(1)=phi*req;
  ans(3)=req;
  for (int fleet=1;fleet<=nfsh;fleet++)
    if (sel_map(1,fleet)==stock)
      ans(2)+=req*sum(elem_prod(wt_fsh(fleet,endyr),
        elem_prod(n,elem_prod(elem_div(ff(fleet),z),1.-surv))));
  RETURN_ARRAYS_DECREMENT();
  return ans;

FUNCTION void get_projection_msy()
  // Locate a feasible maximum, then refine with differentiable Newton steps.
  // The optimizer and final evaluator call exactly the same equilibrium function.
  const double flo=1.e-4;
  const double fhi=5.;
  const double df=1.e-5;
  for (int stock=1;stock<=nstk;stock++)
  {
    double best_f=flo, best_y=-1.;
    int best_index=-1;
    for (int jgrid=0;jgrid<=100;jgrid++)
    {
      dvariable f=flo*exp(log(fhi/flo)*jgrid/100.);
      dvar_vector ans=projection_equilibrium(f,stock);
      if (value(ans(1))>0. && value(ans(3))>0. && value(ans(2))>best_y)
      {
        best_f=value(f); best_y=value(ans(2)); best_index=jgrid;
      }
    }
    if (best_index<=0 || best_index>=100)
    {
      cerr << "Projection MSY has no interior feasible maximum for stock " << stock << endl;
      ad_exit(1);
    }
    dvariable f=best_f;
    for (int iteration=0;iteration<8;iteration++)
    {
      dvariable fp=f+0.5*df, fm=f-0.5*df;
      dvar_vector a=projection_equilibrium(f,stock);
      dvar_vector ap=projection_equilibrium(fp,stock);
      dvar_vector am=projection_equilibrium(fm,stock);
      dvariable slope=(ap(2)-am(2))/df;
      dvariable curvature=(ap(2)+am(2)-2.*a(2))/(0.25*df*df);
      if (value(curvature)>=-1.e-12)
      {
        cerr << "Nonconcave projection MSY refinement for stock " << stock << endl;
        ad_exit(1);
      }
      dvariable next=f-slope/curvature;
      if (value(next)<=flo || value(next)>=fhi)
      {
        cerr << "Projection MSY refinement outside search bounds for stock " << stock << endl;
        ad_exit(1);
      }
      f=next;
    }
    dvar_vector ans=projection_equilibrium(f,stock);
    Fmsy_proj(stock)=f;
    Bmsy_proj(stock)=ans(1);
    MSY_proj(stock)=ans(2);
    msy_proj_status(stock)=1;
  }
...
FUNCTION dvar_vector yld(const dvar_vector& Fratio, const dvariable& Ftmp,int istk,int iyr)
  ...
  for (k=1;k<=nfsh;k++)
  { 
    Fatmp(k) = Fratio(k) * Ftmp * seltmp4(k);
    Ztmp    += Fatmp(k);
  } 
...
FUNCTION dvar_vector yld(const dvar_vector& Fratio, const dvariable& Ftmp, int istk)
  ...
  for (k=1;k<=nfsh;k++)
  { 
    Fatmp(k) = Fratio(k) * Ftmp * seltmp5(k);
    Ztmp    += Fatmp(k);
  } 
  dvar_vector survtmp = mfexp(-Ztmp);
...
FUNCTION dvariable yield(const dvar_vector& Fratio, const dvariable& Ftmp,int istk,int iyr)
  ...
  dvar_matrix seltmp6(1,nfsh,1,nages);
  for (k=1;k<=nfsh;k++)
    if (sel_map(1,k) == istk)
      seltmp6(k) = sel_fsh(k,iyr); // NOTE uses last-year of fishery selectivity for projections.
  ...
  for (k=1;k<=nfsh;k++)
  { 
    Fatmp(k) = Fratio(k) * Ftmp * seltmp6(k);
    Ztmp    += Fatmp(k);
  } 
  dvar_vector survtmp = mfexp(-Ztmp);
...
FUNCTION dvariable yield(const dvar_vector& Fratio, const dvariable& Ftmp,int istk)
  ...
  for (k=1;k<=nfsh;k++)
  { 
    Fatmp(k) = Fratio(k) * Ftmp * seltmp7(k);
    Ztmp    += Fatmp(k);
  } 
  dvar_vector survtmp = mfexp(-Ztmp);
...
// Function to write out data file for new Ianelli 2005 projection model....
 ...
 for (k=1;k<=nfsh;k++)
   for (j=1;j<=nages;j++)
     seltmp2(j) += value(Fratio(k))*(value(sel_fsh(k,endyr,j)) 
 ...
 seltmp2 = value(sel_fsh(1,endyr)) +value(sel_fsh(1,endyr-1))  +value(sel_fsh(1,endyr-2));  
 newproj << seltmp2/max(seltmp2)<<endl;
...
FUNCTION dvar_vector SolveF2(const int& iyr, const dvector&  Catch, const int& istk)
  ...
  dvariable dd = 10.;
  dvariable cc; 
  dvar_matrix  seltmp3(1,nfsh,1,nages);
  ...
  for (k=1;k<=nfsh;k++)
  {
    seltmp3(k)= sel_fsh(k,iyr); // Selectivity
    wt_tmp(k)= wt_fsh(k,iyr); // 
    btmp(k)  =  N_tmp * elem_prod(seltmp3(k),wt_tmp(k));
    hrate(k) = Catch(k)/btmp(k);
    Frat(k)  = Catch(k)/sum(Catch);
    Fratsel(k) = Frat(k)*seltmp3(k);
...
FUNCTION Write_R
  ...
  for (s=1;s<=nstk;s++)
  {
    ...
    R_report<< "$Fmsy_global"<<endl<<Fmsy_global(s)<<endl;
    R_report<< "$Bmsy_global"<<endl<<Bmsy_global(s)<<endl;
    R_report<< "$MSY_global"<<endl<<MSY_global(s)<<endl;
    R_report<< "$Fmsy_proj"<<endl<<Fmsy_proj(s)<<endl;
    R_report<< "$Bmsy_proj"<<endl<<Bmsy_proj(s)<<endl;
    R_report<< "$MSY_proj"<<endl<<MSY_proj(s)<<endl;
    R_report<< "$msy_proj_status"<<endl<<msy_proj_status(s)<<endl;
    R_report<< "$msy_sel_years"<<endl<<endyr-msy_sel<<" "<<endyr-1<<endl;
    R_report<< "$fut_sel_years"<<endl<<(fut_sel>0 ? endyr-fut_sel : endyr)<<" "<<(fut_sel>0 ? endyr-1 : endyr)<<endl;
    ...
    R_report<< "$repl_target_SSB"<<endl<<repl_target_SSB(s)<<endl;
    R_report<< "$repl_SSB_residual"<<endl<<repl_SSB_residual(s)<<endl;
    R_report<< "$repl_status"<<endl<<repl_status(s)<<endl;
    R_report<< "$repl_years"<<endl<<endyr+1<<" "<<endyr+2<<endl; 
    for (int k=1;k<=nfsh;k++)
    {
      if (nyrs_fsh_age(k)>0)
    ...
    R_report   << "$Ftot_fut" << endl<< Ftot_fut<<endl;
