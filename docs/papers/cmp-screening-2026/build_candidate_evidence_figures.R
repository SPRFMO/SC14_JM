...
dynamic_kobe_annual <- rbindlist(lapply(names(candidate_runs), function(run_code) {
  ...
  rbindlist(list(
    ...
    flq_year_iter_table(metric$SB / unfished_ssb$CJM[, as.character(years)],
      run_code, "SSBdB0"),
...
fwrite(catch_events,
  file.path(summary_dir, "candidate_quilt_catch_cut_diagnostics_reference.csv"))
event_catch <- catch_events[, .(mp, iter, year,
  statistic = "CatchDrop20", flag, exclusion)]
event_ssb <- dynamic_kobe_annual[
  statistic == "SSBdB0" & year %in% 2041:2050,
  .(mp, iter, year, statistic = "SSBbelow8dB0",
    flag = fifelse(is.finite(data) & data >= 0,
      as.integer(data < 0.08), NA_integer_))]
event_ssb[, exclusion := fifelse(is.na(flag), "missing_or_invalid", "none")]
event_counts <- rbindlist(list(event_catch, event_ssb))[, .(
  # Pool event counts and valid comparison counts; never average iteration rates.
  events = sum(flag, na.rm = TRUE),
  valid_years = sum(!is.na(flag)),
  missing_years = sum(exclusion == "missing_or_invalid"),
  excluded_crash_years = sum(exclusion == "crash"),
  excluded_edge_years = sum(exclusion == "near_20_percent"),
  no_previous_advice = sum(exclusion == "no_previous_advice"),
  nonconsecutive_years = sum(exclusion == "nonconsecutive_year"),
  available_years = .N
), by = .(mp, iter, statistic)]
stopifnot(all(event_counts[statistic == "SSBbelow8dB0", available_years] == 10L))
event_counts[, frequency := fifelse(valid_years > 0, events / valid_years, NA_real_)]
fwrite(event_counts, file.path(summary_dir, "candidate_quilt_event_counts_reference.csv"))
event_summary <- event_counts[, .(value = sum(events, na.rm = TRUE) / sum(valid_years)),
  by = .(mp, statistic)]

metric_definitions <- data.table(
  statistic = c(
    "SBMSY", "FMSY", "C", "IACC", "VB2025", "VBMSY",
    "SB0red", "PC270", "Creduction", "CatchDrop20", "SSBbelow8dB0"
  ),
  metric = c(
    "SB / SB[MSY]", "F / F[MSY]", "Catch", "IACC",
    "VB / VB[2025]", "VB / VB[MSY]", "P(Kobe red)",
    "P(catch < 270 kt)", "Mean catch reduction",
    "Advice reductions >19% (%)", "SSB <8% dynamic B0 (%)"
  ),
  direction = c(
...
quilt <- rbindlist(list(base_quilt, dynamic_kobe, catch_summary, event_summary),
...
quilt[, display_value := fifelse(
  ...
  fifelse(statistic %in% c("SB0green", "SB0red", "PC270", "SSBbelow8dB0", "CatchDrop20"),
...
quilt_plot <- ggplot(quilt, aes(metric, mp, fill = relative_preference)) +
  ...
  labs(
    ...
    subtitle = paste(
      "Advice reductions use all saved comparisons; other probabilities pool 2041-2050",
...
ggsave(file.path(output_dir, "candidate_quilt_reference.png"),
  quilt_plot, width = 18, height = 7.8, dpi = 160)
