...
The shortlist uses stated thresholds to identify similar alternatives and focus the discussion. Omitted CMPs retain advantages that may matter under other priorities. Small differences in reference-OM indicators are treated as close alternatives. Two cases are less favourable within the specified tolerances; their remaining advantages are stated below. Agreement on acceptable risk remains a separate step. A score of 100 identifies the best selected value; judging its acceptability requires an agreed risk limit.

## Data, periods and indicator definitions

The results cover eight candidate management procedures (CMPs), each with 500 simulations under the reference operating model (OM) and nine alternative OMs used to test performance under different assumptions (robustness tests). Two-stock OMs are shown by biological component; their catches and biomasses are reported separately. All performance calculations in this paper use **2041–2050**, except the inherited mean-catch-reduction column available as an optional scorecard indicator, which uses **2026–2050**. The worm plots span **2025–2050**.
...
The **filtered catch-cut count** counts retained realized-catch reductions greater than 20.1%. Transitions are excluded when SSB is below 8% of dynamic B0 in either the current or previous year, or the reduction is within 0.1 percentage point of 20%. These are diagnostic exclusions requested for this indicator only: the low-SSB indicator, catch and IACC trade-offs, and worm plots retain the crash trajectories. A wholly excluded window has zero retained events. The plotted mean count uses the original ten-year window for every simulation. A probability calculated only among eligible years would use a different denominator. Recorded advice respects the 20% limit within numerical tolerance in the checked HS−20 run; realized-catch shortfalls and numerical edge cases motivated the filter. Checks of the remaining projection calculations are still required.

[Annual exclusions and checks](candidate_quilt_catch_cut_diagnostics_reference.csv) and [counts and eligible years per simulation](candidate_quilt_event_counts_reference.csv) retain the audit trail.

## Screening the CMP set

Five management-facing indicators are used for this screen: median catch, IACC, low-catch probability, SSB-below-previous-Blim frequency, and the filtered catch-cut count. All eleven indicators remain available in the scorecards; the shortlist uses the five indicators listed here.

The following absolute differences are treated as small for **forming this discussion shortlist**. These thresholds remain open for review:
...
The fixed-catch benchmark is available in the reference selection only. The supplied fixed-catch results cover the reference OM. The 270 kt whole-stock catch threshold is omitted from two-stock components. When fixed catch is selected, the two vulnerable-biomass indicators are unavailable pending extraction from the supplied run; deselect it to restore those choices. Each scorecard displays only indicators shared by the selected CMPs. The fixed-catch snapshot uses its own stored reference points and reconstructed dynamic B0, preserving its reported tuning convention. Users can change CMPs, indicators, scaling and weights. The overall score helps explore preferences. The shortlist follows the stated five-indicator comparison.
...
**Zero-best exception:** when the best value for a lower-is-better indicator is zero, zero-valued CMPs score 100 and positive-valued CMPs score zero. This rule is necessary because the ordinary ratio cannot distinguish positive values when its numerator is zero. The filtered-cut count separates the retained CMPs: HS−20 and PR−20 are zero, while HS−30 is positive. Giving this column a substantial weight can therefore impose a sharp penalty. Inspect its raw count and the exclusion definitions before assigning weight. Related stock or stability indicators can overlap even when given equal weights.
...
5. Retain projection-failure diagnostics and advice-versus-realized-catch checks as outstanding validation work. The catch-cut filter changes which events are counted; failed projections and their effects on other indicators still require investigation.
